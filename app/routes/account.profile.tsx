        ...
        <section className='px-10'>
            <div className="md:max-w-[60%]">
                <div className="space-y-6">
                    ...
                    <Separator />
                    <div>
                        <h3 className="text-lg font-bold">Thông tin đào tạo</h3>
                        <p className="text-sm text-muted-foreground">
                            Đây là những thông tin liên quan đến việc học piano của bạn tại trung tâm <strong>Photon Piano</strong>.
                        </p>
                        <Suspense fallback={<LoadingSkeleton />}>
                            <Await resolve={promise}>
                                {({ accountPromise }) => (
...
function ProfileForm() {
    ...
    return <>
        <Form onSubmit={handleOpenConfirmationDialog} method='POST' action='/account/profile' navigate={false}
            className='flex flex-col gap-5'>

            <Avatar className='size-32 inline-block relative left-1/2 -translate-x-1/2'>
                <AvatarImage src={getValues().avatarUrl ? getValues().avatarUrl : "https://github.com/shadcn.png"} alt={account.userName || account.fullName} />
                <AvatarFallback>
