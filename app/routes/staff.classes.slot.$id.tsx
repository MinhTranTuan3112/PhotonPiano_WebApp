...
export default function StaffClassSlotDetail() {
    ...
    return (
        <div className='p-4'>
            <div className='flex flex-col sm:flex-row place-content-between gap-2'>
                <Button variant={'outline'} onClick={() => navigate(-1)}><ArrowLeftCircle className='mr-4' /> Trở về</Button>
                <div className='flex gap-2'>
                    {
                        isEdit ? (
                            <>
                                <Button className='bg-green-500 hover:bg-green-300' onClick={() => navigate(-1)}><CheckIcon className='mr-4' /> Lưu thay đổi</Button>
                                <Button className='bg-red-400 hover:bg-red-200' onClick={() => setIsEditing(false)}><XIcon className='mr-4' /> Hủy thay đổi</Button>
                            </>
                        ) : (
                            <Button variant={'theme'} onClick={() => setIsEditing(true)}><Edit2Icon className='mr-4' /> Chỉnh sửa buổi học</Button>
                        )
                    }
                    <Button variant={'destructive'} onClick={() => navigate(-1)}><Trash className='mr-4' /> Xóa buổi học</Button>
                </div>
            </div>
            <div className="p-6 bg-white shadow-lg rounded-lg mt-10">
                <Suspense fallback={<Skeleton className="text-center h-96 w-full">Loading slot details...</Skeleton>}>
                    <Await resolve={slotPromise}>
                        {(data) => (
                            <SlotDetailComponent slot={data.slot} isEdit={isEdit} idToken={idToken} />
                        )}
                    </Await>
...
function SlotDetailComponent({ slot, isEdit, idToken }: { slot: SlotDetail, isEdit: boolean, idToken: string }) {
    
    const [selectedRoomId , setSelectedRoomId] = useState(slot.roomId)
        ...
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <DetailItem label="Ngày" labelIcon={<CalendarDays />} value={
                    isEdit ? (
                        <DatePickerInput className='mt-2 w-64' value={new Date(slot.date)} />
                    ) : (
                ...
                <DetailItem label="Ca học" labelIcon={<Clock />} value={
                    isEdit ? (
                        <Select defaultValue={slot.shift.toString()} >
