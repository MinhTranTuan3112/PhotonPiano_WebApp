  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        expandIconTheme:
          "group relative text-theme-foreground bg-theme hover:bg-theme/90",
        ringHover:
    ...
    return (
      <Comp
        className={cn(
          ...
          isLoading && 'bg-primary text-primary-foreground hover:bg-primary/90'
        )}
        ref={ref}
        {...props}
      >
        {isLoading ? (
          ...
          Icon && iconPlacement === 'left' && (
            <div
              className={cn({
                'w-0 translate-x-[0%] pr-0 opacity-0 transition-all duration-200 group-hover:w-5 group-hover:translate-x-100 group-hover:pr-2 group-hover:opacity-100': isExpandIconVariant,
                'pr-2': !isExpandIconVariant,
              })}
            >
              <Icon />
            </div>
        ...
        {!isLoading && Icon && iconPlacement === 'right' && (
          <div
            className={cn({
              'w-0 translate-x-[100%] pl-0 opacity-0 transition-all duration-200 group-hover:w-5 group-hover:translate-x-0 group-hover:pl-2 group-hover:opacity-100': isExpandIconVariant,
              'pl-2': !isExpandIconVariant,
            })}
          >
            <Icon />
          </div>
