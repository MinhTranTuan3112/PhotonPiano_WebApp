...
export const adminColumns: ColumnDef<Account>[] = [
    {
        accessorKey: 'Avatar',
        header: 'Ảnh',
        cell: ({ row }) => {
...
function ActionsDropdown({ table, accountId, status }: {
    ...
    const { open: handleOpenModal, dialog: confirmDialog } = useConfirmationDialog({
        title: 'Confirm Ungranting',
        description: 'Do you want to remove this admin? This account will become a staff account',
        onConfirm: () => {
            handleUngrant();
        }
    })
